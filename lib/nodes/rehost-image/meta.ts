...
export const rehostImageMeta: NodeMeta<RehostImageConfig> = {
  ...
  category: "transform",
  // Generic input for explicit, visible flow only — the source URL is bound by
  // token (see `source`), and the engine orders this node after anything that
  // token references, so the wire is about clarity, not carrying the data.
